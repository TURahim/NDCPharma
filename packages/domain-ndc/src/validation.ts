 ...
 */
export function validateNDCFormat(ndc: string): NDCValidationResult {
  const errors: string[] = [];
  ...
  const isValidFormat = 
    NDC_FORMATS.WITH_DASHES.test(trimmed) ||
    NDC_FORMATS.WITHOUT_DASHES.test(trimmed) ||
    NDC_FORMATS.TEN_DIGIT.test(trimmed);

  if (!isValidFormat) {
    errors.push(
      'Invalid NDC format. Expected 10 or 11 digits, optionally with dashes (XXXXX-XXXX-XX)'
    );
    return {
      isValid: false,
      errors,
  ...
  // Normalize NDC
  let normalizedNdc: string;
  try {
    normalizedNdc = normalizeNDC(trimmed);
  } catch (error) {
    errors.push(`Failed to normalize NDC: ${(error as Error).message}`);
    return {
      isValid: false,
      errors,
      warnings,
 ...
 */
export function validateNDCWithStatus(
  ndc: string,
...
export function normalizeNDC(ndc: string): string {
  ...
  if (digits.length < 10 || digits.length > 11) {
    throw new Error(
      `Invalid NDC length: ${digits.length} digits. Expected 10 or 11 digits.`
    );
  }
 ...
 */
export function extractProductNDC(packageNdc: string): string {
 ...
 */
export function isValidProductNDC(productNdc: string): boolean {
 ...
 */
export function areNDCsEqual(ndc1: string, ndc2: string): boolean {
  try {
 ...
 */
function calculateDaysUntilDate(dateString: string): number | null {
  try {
 ...
 */
export function isStandardFormat(ndc: string): boolean {
 ...
 */
export function parseNDCSegments(ndc: string): NDCSegments {
 ...
 */
export function validateNDCBatch(
  ndcs: string[]
): Map<string, NDCValidationResult> {
  const results = new Map<string, NDCValidationResult>();
 ...
 */
export function filterValidNDCs(ndcs: string[]): string[] {
  const valid: string[] = [];
