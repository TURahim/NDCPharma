...
export function chooseBestPackage(
  ...
  const sortedPackages = [...packages].sort(
    (a, b) => a.packageSize.quantity - b.packageSize.quantity
  );
  ...
  if (adequatePackage) {
    ...
    if (overfillPct > 20) {
      warnings.push(
        `Significant overfill: ${overfillPct.toFixed(1)}% (${overfill} extra ${adequatePackage.packageSize.unit}). ` +
  ...
  warnings.push(
    `No package meets required quantity. Largest available is ${largestPackage.packageSize.quantity} ${largestPackage.packageSize.unit}. ` +
 ...
 * @returns Best package selection with metadata
 */
export function selectLiquidPackages(
  packages: PackageCandidate[],
  requiredML: number
): PackageSelection {
  const warnings: string[] = [];
  ...
  const sortedPackages = [...normalizedPackages].sort(
    (a, b) => a.normalizedQuantityML - b.normalizedQuantityML
  );
  
  // Strategy 1: Find exact match (within 1 mL tolerance for rounding)
  const exactMatch = sortedPackages.find(
    pkg => Math.abs(pkg.normalizedQuantityML - requiredML) < 1
  );
  ...
  const minimalOverfillPackage = sortedPackages.find(
    pkg => {
  ...
  const adequatePackage = sortedPackages.find(
    pkg => pkg.normalizedQuantityML >= requiredML
  );
  ...
  if (adequatePackage) {
    ...
    if (overfillPct > 20) {
      warnings.push(
        `Significant overfill: ${overfillPct.toFixed(1)}% (${overfill.toFixed(1)} mL extra). ` +
        `Patient will have leftover medication. Consider discussing with prescriber.`
      );
    }
  ...
  warnings.push(
    `No single bottle meets required volume. Largest available is ${largestPackage.packageSize.quantity} ${largestPackage.packageSize.unit}. ` +
