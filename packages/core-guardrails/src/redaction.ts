 ...
 */
export function redactObjectPHI<T>(obj: T): T {
  if (!obj) return obj;
  
  if (typeof obj === 'string') {
    return redactPHI(obj) as unknown as T;
  }
  
  if (Array.isArray(obj)) {
    return obj.map(item => redactObjectPHI(item)) as unknown as T;
  }
 ...
 */
function isKnownPHIField(fieldName: string): boolean {
  const phiFields = [
    'patientName',
    'patient_name',
    'firstName',
 ...
 */
export function createSafeCacheKey(...parts: (string | number | boolean)[]): string {
