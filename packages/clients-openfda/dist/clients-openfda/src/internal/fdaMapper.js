...
exports.mapFDAResultToNDCPackage = mapFDAResultToNDCPackage;
exports.mapFDAResultToNDCDetails = mapFDAResultToNDCDetails;
exports.parsePackageSize = parsePackageSize;
exports.normalizeUnit = normalizeUnit;
exports.normalizeNDC = normalizeNDC;
exports.normalizeDosageForm = normalizeDosageForm;
exports.mapActiveIngredients = mapActiveIngredients;
exports.parseMarketingStatus = parseMarketingStatus;
exports.parseFDADate = parseFDADate;
exports.extractRxCUI = extractRxCUI;
exports.extractConcentration = extractConcentration;
exports.filterByDosageForm = filterByDosageForm;
exports.filterActivePackages = filterActivePackages;
exports.sortByPackageSize = sortByPackageSize;
exports.groupByDosageForm = groupByDosageForm;
const _core_guardrails_1 = require("@core-guardrails");
// Import concentration parser (will be available after domain-ndc is built)
let parseConcentration = null;
let isConcentrationString = null;
...
try {
    ...
    _core_guardrails_1.logger.debug('Concentration parser not available', { error: error });
}
/**
 ...
 */
function mapFDAResultToNDCPackage(fdaResult) {
    const packages = [];
    ...
    for (const packaging of fdaResult.packaging || []) {
        try {
            ...
            _core_guardrails_1.logger.warn(`Failed to map FDA package: ${packaging.package_ndc}`, {
                error: error,
 ...
 */
function mapFDAResultToNDCDetails(fdaResult) {
    try {
        // Get the first package as the primary package
        const primaryPackaging = fdaResult.packaging?.[0];
        if (!primaryPackaging) {
            _core_guardrails_1.logger.warn('FDA result has no packaging information', {
                productNdc: fdaResult.product_ndc,
            });
            return null;
        }
        ...
        _core_guardrails_1.logger.error('Failed to map FDA result to NDC details', error, {
            productNdc: fdaResult.product_ndc,
        });
        return null;
    }
...
function parsePackageSize(description) {
    ...
    _core_guardrails_1.logger.warn(`Unable to parse package size: "${description}"`);
    return {
        quantity: 1,
        unit: 'UNKNOWN',
        description,
 ...
 */
function normalizeDosageForm(dosageForm) {
 ...
 */
function mapActiveIngredients(ingredients) {
 ...
 */
function extractConcentration(fdaResult) {
    ...
    try {
        ...
        _core_guardrails_1.logger.warn('Failed to extract concentration', {
            error: error,
            productNdc: fdaResult.product_ndc,
 ...
 */
function filterByDosageForm(packages, dosageForm) {
 ...
 */
function groupByDosageForm(packages) {
