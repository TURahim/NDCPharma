...
import { DrugNormalizer } from "../src/internal/normalizer";
import { RxNormService } from "../src/internal/rxnormService";
import { RxCUINotFoundError, DrugNotFoundError } from "@core-guardrails";

// Create mock RxNorm service
const mockRxNormService = {
  searchByName: vi.fn(),
  getApproximateMatches: vi.fn(),
  getSpellingSuggestions: vi.fn(),
  getRxCUIProperties: vi.fn(),
...
describe("DrugNormalizer", () => {
  ...
  describe("normalizeDrug", () => {
    describe("Exact Match Strategy", () => {
      it("should normalize drug via exact match", async () => {
        ...
        (mockRxNormService.searchByName as any).mockResolvedValue({
          idGroup: {
            name: "LISINOPRIL",
            rxnormId: ["104377"],
          },
        ...
        (mockRxNormService.getRxCUIProperties as any).mockResolvedValue({
          properties: {
            rxcui: "104377",
            name: "LISINOPRIL",
            tty: "IN",
    ...
    describe("Approximate Match Strategy", () => {
      it("should fall back to approximate match when exact fails", async () => {
        ...
        (mockRxNormService.getApproximateMatches as any).mockResolvedValue({
          approximateGroup: {
            candidate: [
        ...
        (mockRxNormService.getRxCUIProperties as any)
      ...
      it("should return up to 4 alternatives", async () => {
        ...
        for (let i = 1; i <= 10; i++) {
          (mockRxNormService.getRxCUIProperties as any).mockResolvedValueOnce({
            properties: {
              rxcui: `${i}`,
              name: `DRUG ${i}`,
              tty: "IN",
    ...
    describe("Spelling Suggestion Strategy", () => {
      it("should fall back to spelling suggestions when approximate fails", async () => {
        ...
        (mockRxNormService.getSpellingSuggestions as any).mockResolvedValue({
          suggestionGroup: {
            suggestionList: {
        ...
        (mockRxNormService.searchByName as any).mockResolvedValueOnce({
      ...
      it("should try multiple spelling suggestions", async () => {
        ...
        (mockRxNormService.searchByName as any)
  ...
  describe("normalizeDrugByRxCUI", () => {
    ...
    it("should handle API errors gracefully", async () => {
      (mockRxNormService.getRxCUIProperties as any).mockRejectedValue(
        new Error("API Error")
      );
