 ...
 */
exports.SYSTEM_PROMPT = `You are an expert pharmaceutical AI assistant specializing in NDC (National Drug Code) package selection and prescription fulfillment. Your role is to recommend the most appropriate NDC packages for dispensing medications based on prescription details.
 ...
 */
function generateUserPrompt(request) {
...
function validateResponseStructure(response) {
    ...
    if (!response.primaryRecommendation ||
    ...
    if (response.costEfficiency) {
        if (typeof response.costEfficiency.estimatedWaste !== 'number' ||
