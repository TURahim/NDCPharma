...
export class OpenAIService {
  ...
  private logger = createLogger({ service: 'OpenAIService' });
  ...
  constructor(config: OpenAIServiceConfig = {}) {
    ...
    if (this.config.apiKey && FEATURE_FLAGS.ENABLE_OPENAI) {
      try {
  ...
  async getRecommendation(
    ...
    try {
      ...
      let recommendation: NDCRecommendationResponse;
      try {
      ...
      if (!validateResponseStructure(recommendation)) {
        throw new Error('Invalid response structure from OpenAI');
      }

      // Calculate usage metrics
      const usage: OpenAIUsageMetrics = this.calculateUsage(
      ...
      this.logger.error('OpenAI recommendation failed', error as Error, {
        executionTime,
  ...
  private calculateUsage(
    ...
    const model = this.config.model as keyof typeof this.PRICING;
   ...
   */
  private handleError(error: unknown): OpenAIError {
    if (error instanceof OpenAI.APIError) {
      let type: OpenAIError['type'] = 'api_error';
