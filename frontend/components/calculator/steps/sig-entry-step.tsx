...
import { 
  ...
  Clock, Pill, Calendar, Search, X, Info, Copy,
  ToggleLeft, ToggleRight, Sparkles, History, Trash2
} from 'lucide-react';
...
export function SIGEntryStep() {
  ...
  // Check if package is selected (required context)
  ...
  const validateStructured = useCallback(() => {
    ...
    if (!daysSupply || parseInt(daysSupply) <= 0) {
    ...
    if (selectedPackage?.dosageForm && unit) {
      ...
      if (dosageForm.includes('liquid') || dosageForm.includes('solution') || dosageForm.includes('syrup')) {
        if (unit !== 'mL') {
          newErrors.unit = `Expected 'mL' for liquid dosage form, got '${unit}'`;
        }
      } else if (dosageForm.includes('inhaler') || dosageForm.includes('aerosol')) {
  ...
  }, [dose, frequency, unit, daysSupply, selectedPackage]);
  ...
  const saveSIG = useCallback(() => {
    ...
    setRecentSIGs(getRecentSIGs());
  }, [mode, dose, frequency, unit, daysSupply, freetext, validate, dispatch, selectedPackage]);
  
  /**
   * Apply template
   */
  const applyTemplate = useCallback((template: SIGTemplate) => {
    if (mode === 'structured') {
  ...
  const sigPreview = useMemo(() => {
    if (mode === 'structured') {
      ...
      const frequencyLabel = FREQUENCY_OPTIONS.find(f => f.value === frequency)?.label || `${frequency} times daily`;
      return `Take ${dose} ${unit}${parseFloat(dose) > 1 ? 's' : ''} ${frequencyLabel.toLowerCase()} for ${daysSupply} days`;
    } else {
  ...
  if (!selectedPackage) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Enter Prescription Directions</h2>
        ...
        <Card className="border-2 border-amber-300 bg-amber-50">
          <div className="p-8 text-center">
            ...
            <p className="text-amber-900 font-medium mb-2">Package Not Selected</p>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-blue-900 mb-1">Selected Package</p>
              <div className="text-sm text-blue-800">
                <span className="font-medium">{selectedPackage.brandName || selectedPackage.genericName}</span>
                {' • '}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {sig.drugName && (
                            <span className="text-xs text-gray-500">{sig.drugName}</span>
                          )}
                          <Badge variant="outline" className="text-xs">
                            {sig.mode === 'structured' ? 'Structured' : 'Free-text'}
                          </Badge>
                      ...
                      {templates.map(template => (
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500">{template.daysSupply} days supply</p>
                          </div>
                        </CommandItem>
                      ))}
                    </CommandGroup>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    className={`flex-1 ${errors.daysSupply ? 'border-red-500' : ''}`}
                  />
          ...
          <div className="p-6 space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                Enter the complete prescription directions as they should appear on the label
              </p>
            </div>
      ...
      {sigPreview && (
        <Card className={`border-2 ${Object.keys(errors).length === 0 ? 'border-green-500 bg-green-50' : 'border-gray-300 bg-gray-50'}`}>
          <div className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-gray-900 mb-1">SIG Preview</p>
                <p className="text-sm text-gray-800 italic">"{sigPreview}"</p>
                
                {mode === 'structured' && calculatedQuantity && (
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    <p className="text-xs text-gray-600">
                      Estimated Total Quantity: <span className="font-semibold text-gray-900">{calculatedQuantity} {unit}{calculatedQuantity > 1 ? 's' : ''}</span>
                    </p>
                  </div>
                )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-900 space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-blue-800">
              <li><strong>Structured Mode:</strong> Use for precise quantity calculations</li>
              <li><strong>Free-Text Mode:</strong> Use for complex or custom directions</li>
              <li><strong>Templates:</strong> Quick-fill common prescription patterns</li>
